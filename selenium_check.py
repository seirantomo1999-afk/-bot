...
ONLY_HOLIDAYS = True          # ★休日（祝日含む）だけを表示
NEXT_WEEKS_TO_CHECK = 8       # 今週＋8週＝計9週
# =================
...
def scrape_week_A_holidays_only(tbody):
    """この週の A_セル（空き）だけを拾い、休日のみで [(date, ymd, hour)] を返す"""
    results = []
...
def run_for_park(driver, wait, park_keyword: str):
    ...
    # 公園リスト読み込み待ち（ロード直後は option が1件(空)のことがある）
    sel_park = Select(park_el)
    for _ in range(20):
        if len(sel_park.options) > 1: break
        time.sleep(0.3)
